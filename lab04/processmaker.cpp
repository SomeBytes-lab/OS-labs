...
void print_array_int(int* arr_ptr)
{
   for (int i = 0; i < 20; i++)
      printf("%i ", *(arr_ptr + i));
   printf("\n");
...
{
   qsort(shared_mem_ptr, 20, 4, compare_int_value);
   print_array_int(shared_mem_ptr);
   exit(0);
}
...
{
   int mem_id;
   int* shared_mem_ptr = (int*) allocate_shared_memory(80, mem_id);
   printf("mem_id = %d\n", mem_id);
